...
## 2.7.21 (Latest, WebView2)

Стабильный канал. Десктоп на WebView2; установщик `CoronerChat.Setup.*.exe`. Папка `CoronerChat-data` сохраняется.

- Twitch: slash-команды и бот-команды вроде `/нотификейшен офф` уходят целиком (Helix больше не режет `/foo bar` до `bar`)
- Поле ввода: Enter больше не подменяет уже набранные аргументы автодополнением
- Значки и эмоуты в ленте масштабируются с кеглем текста и стоят на одной оси с серединой строки
- Команды `/*` уходят в подключённый канал Twitch с твоего ориг-аккаунта; чат принимает действие, только если автор — вошедший Twitch-логин (или `coroner_of_autopsy`)
- У стримера сырой `/*` не идёт в ленту и OBS overlay: короткое `admin: уведомления off`
- Twitch: кастомные VIP и уровни подписок канала, не глобальная звезда

## 2.7.20 (WebView2)

Стабильный канал. Десктоп на WebView2; установщик тот же `CoronerChat.Setup.*.exe`. Папка `CoronerChat-data` сохраняется.

- Twitch: гифки из GIF Keyboard (Giphy) показываются в чате картинкой, не текстом `[название GIF]`
- Twitch: смайлы и значки перед ником снова грузятся, если прокси CDN не ответил (прямой static-cdn)
- YouTube URL-чат: одиночные сбои опроса `live_chat` через прокси больше не спамят «подключен / отключен»
- VK Video Live: вход через встроенное окно ловит Bearer с сетевых запросов WebView2 и не висит минутами после логина

## 2.7.19 (WebView2)

Стабильный канал на момент 10.09.2026. Десктоп на WebView2; установщик тот же `CoronerChat.Setup.*.exe`. Папка `CoronerChat-data` сохраняется.

- YouTube URL-чат: одиночные сбои опроса `live_chat` через прокси больше не спамят «подключен / отключен»
- VK Video Live: вход через встроенное окно ловит Bearer с сетевых запросов WebView2 (как Electron) и не висит минутами после логина
- VK Video Live: окно авторизации больше не срывается на «Окно VK закрыто» / `rpc.url is not a function`
- Настройки в отдельном окне, шапка с чипами площадок — как в 2.7.18

## 2.7.18 (prerelease, WebView2)

Предпросмотр десктопа на WebView2 (Tauri). Тогда **GitHub Latest не менялся** — стабильный канал оставался Electron **2.7.16**. Этот тег — pre: в приложении Настройки → Обновления → канал «включая prerelease».

- Оболочка WebView2 вместо Electron; данные по-прежнему в `CoronerChat-data`
- Настройки в отдельном окне, поиск в левом меню под «Система»
- Шапка: чипы площадок, шестерёнка не обрезается при узком окне
- Обновление: «Скачать и установить» само закрывает чат, ставит Setup и открывает новую версию
- YouTube URL-чат и Kick через браузер ещё не закрыты как «пережитый эфир»

Не ставь 2.7.18 поверх 2.7.16, если хочешь остаться на Electron — для перехода используй **2.7.21 Latest**.

## 2.7.16

- YouTube: быстрее чат — после первого снимка опрос `get_live_chat` (JSON), без повторной загрузки всей HTML-страницы.
- YouTube: обычные смайлы больше не пропадают; кастомные чуть меньше; лого в значке поднято на 2px.
- YouTube: по ссылке без авторизации после конца эфира чат сам отключается, без спама обрывов.
- YouTube: счётчик показывает онлайн-зрителей, а не накопительные просмотры.
